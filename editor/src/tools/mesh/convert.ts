import { AbstractMesh, VertexBuffer, VertexData, Matrix, Quaternion, Vector3, Skeleton } from "babylonjs";
...
export function convertGltfMeshesToLeftHanded(meshes: AbstractMesh[]) {
	...
	const computedSkeletons: Skeleton[] = [];
	...
	meshes.forEach((mesh) => {
		...
		if (mesh.skeleton && !computedSkeletons.includes(mesh.skeleton)) {
			computedSkeletons.push(mesh.skeleton);

			// TODO: handle bones
			mesh.skeleton.bones.forEach((bone) => {
				bone.setRestMatrix(bone.getRestMatrix().multiply(matrix));
			});
		}
	});
}
